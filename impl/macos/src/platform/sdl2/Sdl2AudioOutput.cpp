...
bool Sdl2AudioOutput::Open(uint32_t sample_rate, uint8_t channels, uint8_t bits) {
    ...
    if (!(SDL_WasInit(0) & SDL_INIT_AUDIO)) {
        if (SDL_InitSubSystem(SDL_INIT_AUDIO) < 0) {
            AA_LOG_E() << "[Sdl2AudioOutput] SDL_INIT_AUDIO init failed: " << SDL_GetError();
            return false;
        }
    }
    ...
    switch (bits) {
        ...
        default:
            AA_LOG_E() << "[Sdl2AudioOutput] Unsupported bit depth: " << (int)bits;
            return false;
    }
    ...
    if (device_id_ == 0) {
        AA_LOG_E() << "[Sdl2AudioOutput] SDL_OpenAudioDevice failed: " << SDL_GetError();
        return false;
    }

    // Open in paused state; Start() will unpause when MEDIA_START arrives
    SDL_PauseAudioDevice(device_id_, 1);
    AA_LOG_I() << "[Sdl2AudioOutput] Opened (paused) - "
...
void Sdl2AudioOutput::Start() {
    if (device_id_ != 0) {
        ...
        AA_LOG_I() << "[Sdl2AudioOutput] Playback started";
    }
}
...
void Sdl2AudioOutput::Close() {
    if (device_id_ != 0) {
        ...
        device_id_ = 0;
        AA_LOG_I() << "[Sdl2AudioOutput] Closed";
    }
}
...
void Sdl2AudioOutput::PushAudioData(const std::vector<uint8_t>& data) {
    ...
    if (SDL_QueueAudio(device_id_, data.data(), static_cast<Uint32>(data.size())) != 0) {
        AA_LOG_W() << "[Sdl2AudioOutput] SDL_QueueAudio failed: " << SDL_GetError();
    }
}
